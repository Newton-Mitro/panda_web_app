...
import { PaginatedJobCircular } from '../../types/job_circular';

interface CareerProps {
    careers: PaginatedJobCircular;
}

const Index: React.FC<CareerProps> = ({ careers }) => {
        ...
        <AppLayout breadcrumbs={breadcrumbs}>
            ...
            <div className="p-6">
                <div className="mb-4 flex flex-col items-start justify-between gap-2 sm:flex-row">
                    <HeadingSmall title="Careers" description="Manage careers" />
                    <Link
                    ...
                    <table className="w-full border-collapse">
                        <thead className="sticky top-0 hidden bg-gray-50 md:table-header-group dark:bg-gray-800">
                            <tr>
                                ...
                                <th className="border-b p-2 text-left dark:border-gray-700">Salary Range</th>
                                <th className="border-b p-2 text-left dark:border-gray-700">Deadline</th>
                        ...
                        <tbody className="flex flex-col md:table-row-group">
                            {careers.data.map((career) => (
                                ...
                                >
                                    <td className="px-2 py-1">{career.title}</td>
                                    <td className="px-2 py-1">{career.location || '-'}</td>
                                    <td className="px-2 py-1">{career.salary_range || '-'}</td>
                                    <td className="px-2 py-1">{career.deadline || '-'}</td>
                ...
                <div className="mt-4 flex flex-col items-center justify-between gap-2 md:flex-row">
                    <span className="text-sm text-gray-600 dark:text-gray-400">Showing {careers.data.length} results</span>
